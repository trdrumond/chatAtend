...
if (($_POST['nome_campo'] ?? '') != '') {
    ...
    $sql_v = "SELECT desc_campo from tbl_forms_pos_input_campo where desc_campo=? and fila_id=?";
    ...
    if (($verifica_nome['desc_campo'] ?? '') == '') {
        $sql = "SELECT count(*) as qtd from tbl_forms_pos_input_campo_cnf where fila_id=?";
        $stm = $PDO->prepare($sql);
        $stm->execute([$idFila]);
        ...
        $sql = "INSERT INTO tbl_forms_pos_input_campo (desc_campo, nome_campo, input_id, fila_id) VALUES (?, ?, ?, ?)";
        $stmt = $PDO->prepare($sql);
        $result = $stmt->execute([$nomeCampo, $nomeCampoDb, $idInput, $idFila]);

        if ($result == 1) {
            $sql = "SELECT id_campo from tbl_forms_pos_input_campo where desc_campo=? and fila_id=?";
            $stm = $PDO->prepare($sql);
            $stm->execute([$nomeCampo, $idFila]);
            $info_campo = $stm->fetch(PDO::FETCH_ASSOC);

            $sql = "SELECT tipo_input, parametro from tbl_forms_pos_input where id_input=?";
            $stm = $PDO->prepare($sql);
            $stm->execute([$idInput]);
            $info_input = $stm->fetch(PDO::FETCH_ASSOC);

            $sql = "INSERT INTO tbl_forms_pos_input_campo_cnf (campo_id, fila_id, input_id, ordem) VALUES (?, ?, ?, ?)";
            $stmt = $PDO->prepare($sql);
            $result = $stmt->execute([$info_campo['id_campo'] ?? null, $idFila, $idInput, $ordem]);
            if ($result == 1) {
                $tablePos = 'tbl_in_pos_' . $idFila . '_' . (int) ($ctt['contrato_id'] ?? 0);
                if (preg_match('/^tbl_in_pos_\d+_\d+$/', $tablePos)) {
                    $add = "ADD COLUMN `" . $nomeCampoDb . "` " . ($info_input['parametro'] ?? '') . ";";
                    $alterTable = "ALTER TABLE `{$tablePos}` {$add}";
                    ...
                    if ($result == 1) {
                        ...
                        <script>
                            load(<?= $idFila ?>);
                            $("#fila_id_<?= $idFila ?>").val('');
                            $("#nome_campo_<?= $idFila ?>").val('');

                            function load(id_filas){
                                $("#tbl_<?= $idFila ?>").html('<div id="load_gif"><img src="img/loading.gif" alt="Carregando..." width="100"></div>');
                                $.post("staff/pos_tbl_config_form.php",
                                {
                                    id_filas: id_filas
                                },
                                function (valor) {
                                    $("#tbl_<?= $idFila ?>").html(valor);
                                });
                            }
                        </script>
                        <?php
