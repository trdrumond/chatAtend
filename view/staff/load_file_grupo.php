...
if($_FILES['arquivo']['size'] > $size_max){
    ...
    if($_FILES['arquivo']['error']!==0){
        ...
        //echo "<script>setTimeout(function () {zera_file()}, 2000);</script>";
            ...
            $ext=end($ext);
            $nameFile = $_POST['group']. "-".strtotime(date('Y-m-d H:i:s'))."." . $ext;
            $pat = '../file/';
            ...
            echo "<script>$('#ipt_file_".$group."').val('".$valorFile."');</script>";
            echo "<script>$('#save_file_".$group."').prop( 'disabled', false );</script>";
