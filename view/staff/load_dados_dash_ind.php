...
<?php
        ...
        $sqlVer="SELECT filas from tbl_user_filas where user_id=?";
        ...
        if($contratoIdPost != 0){
            ...
            $filaIds = [];
            if($nivel_usu==4){
                $filaBind = stSqlInBind(stParseIdCsv((string) $filasConfig));
                $qryBko = ' and fila_id IN (' . $filaBind['ph'] . ')';
                $filaIds = $filaBind['ids'];
            }
            for ($i = 0; $i < 4; $i++) {
                $dashIndParams = array_merge($dashIndParams, $cttBind['ids'], $filaIds);
            }
            $dashIndParams = array_merge($dashIndParams, $cttBind['ids'], $cttBind['ids']);
        }
        ...
        $stmt = $PDO->prepare($sql);
        $result = $stmt->execute($dashIndParams);
...
<?php
    ...
    if($nivel_usu<5){
        ...
        if($infoUser['nivel_id']==4){
            $filaIdUser = (int) $infoUser['fila_id'];
            ...
            $stmt = $PDO->prepare($sql);
            $result = $stmt->execute([$filaIdUser]);
        } else {
    ...
    if($nivel_usu==5){
        $sql_pend = "SELECT count(id_pend) as qtd FROM tbl_pend_info where situacao_id=3 and ate_resp=? and data_hora_fim is null";
        $stmt = $PDO->prepare($sql_pend);
        $result = $stmt->execute([$idu]);
    } else {
        $sql_pend = "SELECT count(id_pend) as qtd FROM tbl_pend_info where situacao_id=3 and data_hora_fim is null";
        $stmt = $PDO->prepare($sql_pend);
        $result = $stmt->execute();
    }
    ...
    if($nivel_usu==5){
        $sql="SELECT id_pend, chat_id, (SELECT protocolo from tbl_chat_fila where id_chat_fila=chat_id) as protocolo FROM tbl_pend_info where situacao_id=3 and ate_resp=? and data_hora_fim is null";
        //echo "<br>".$sql;
        $stmt = $PDO_LOAD->prepare($sql);
        $result = $stmt->execute([$idu]);
        $dadosContratos = $stmt->fetchAll( PDO::FETCH_ASSOC );
        ...
        $sql="SELECT id_pend, chat_id, (SELECT protocolo from tbl_chat_fila where id_fila_chat=chat_id) as protocolo, (SELECT id_chat from tbl_chat_info where fila_chat_id=chat_id) as id_chat FROM tbl_pend_info where situacao_id=3 and ate_resp=? and data_hora_fim is null";
        //echo "<br>".$sql;
        $stmt = $PDO_LOAD->prepare($sql);
        $result = $stmt->execute([$idu]);
        $dadosContratos = $stmt->fetchAll( PDO::FETCH_ASSOC );
...
$idUserStar = (int) $infoUser['id_user'];
...
$stmt = $PDO->prepare($sql);
$result = $stmt->execute([$idUserStar]);
