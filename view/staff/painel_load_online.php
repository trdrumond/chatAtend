...
$sql_fila = '';
if ($idFila != 0) {
    $sql_fila = "and fila_id=? and contrato_id=?";
    $sqlFilaParams = [$idFila, $idContrato];
}

    $sql="SELECT count(*) as qtd, timediff(curtime(), date_format(data_hora, '%H:%i:%s')) as tempo_decorrido from tbl_chat_fila where status_fila=1 and fila_id=?";
...
$stmt = $PDO_LOAD->prepare($sql);
$result = $stmt->execute($sqlFilaParams);
...
for($y=0;$y<count($dds);$y++){
    $idUserLoop = (int) $dds[$y]['id_user'];
    $dds[$y]['nome_completo']=ucwords((strtolower($dds[$y]['nome'])).' '.(strtolower($dds[$y]['sobrenome'][0]))).".";
    $sqlOnline = "SELECT user_id, date_out, date_up FROM tbl_log_diario where data_log=curdate() and user_id=?";
    //echo "<br>".$sqlOnline;
    $stmt = $PDO_LOAD->prepare($sqlOnline);
    $result = $stmt->execute([$idUserLoop]);
    $ddOnline = $stmt->fetch( PDO::FETCH_ASSOC );

    $sqlAtendAtivo ="SELECT id, date_in from tbl_tma_atend where resp_id=? and date_format(date_disp, '%Y-%m-%d')=curdate() and date_format(date_in, '%Y-%m-%d')=curdate() and date_out is null order by date_in asc limit 1";
    ...
    $sqlAtend ="SELECT acao, data_hora, (timediff(now(), data_hora)) as tempo FROM tbl_log_atendimento where user_id=? and date_format(data_hora, '%Y-%m-%d')=curdate() and acao IN ('Login', 'Disponivel', 'Indisponivel', 'Tratamento', 'Logout', 'Pausa', 'Pos') $qry order by data_hora desc limit 1";
    //echo "<br>". $sqlAtend;
    ...
    $sqlAtendimento ="SELECT count(id_fila_chat) as qtd FROM tbl_chat_fila where bko_resp=? AND ".stFilaSqlAtendimentoAtivo();
    //echo "<br>". $sqlAtend;
    ...
    $load = "loadInfoUser('".$idUserLoop."', '".$idContrato."', '".$idFila."')";
    echo "<script>
