...
$sqlVer = "SELECT user_id FROM tbl_pause WHERE date_format(hora_in, '%Y-%m-%d')=curdate() AND hora_out IS NULL AND user_id=".$_SESSION['dados']['id_user']." LIMIT 1";
...
$sqlQtd = "SELECT id, date_disp FROM tbl_tma_atend WHERE date_format(date_disp, '%Y-%m-%d')=curdate() AND fila_chat_id IS NULL AND date_out IS NULL AND resp_id=".$userId." ORDER BY id DESC LIMIT 1";
$stmt = $PDO->prepare($sqlQtd);
$stmt->execute();
$userInfoTma = $stmt->fetch(PDO::FETCH_ASSOC);

if (empty($userInfoTma['id'])) {
    $PDO->prepare("INSERT INTO tbl_tma_atend (resp_id, contrato_id, date_disp) VALUES ('".$userId."', '".$infoUser['id_contrato']."', now())")->execute();
    $stmt->execute();
    $userInfoTma = $stmt->fetch(PDO::FETCH_ASSOC);
}
...
$sqlQtd = "SELECT id FROM infoQtd WHERE date_disp < '".$dateDisp."' AND fila_id=".$idFila;
$stmt = $PDO->prepare($sqlQtd);
$stmt->execute();
...
$buscaFila = function () use ($PDO, $idFila, $userId, $contratoId) {
    $emFila = [];
    if ($idFila > 0) {
        $sqlVer = "SELECT id_fila_chat, protocolo FROM tbl_chat_fila WHERE status_fila=1 AND contrato_id=".$contratoId." AND fila_id=".$idFila." AND (bko_resp IS NULL OR bko_resp='' OR bko_resp=0) ORDER BY id_fila_chat ASC LIMIT 1";
        $stmt = $PDO->prepare($sqlVer);
        $stmt->execute();
        $emFila = $stmt->fetch(PDO::FETCH_ASSOC) ?: [];
    }
    if (empty($emFila['id_fila_chat'])) {
        $stmt = $PDO->prepare("SELECT filas FROM tbl_user_filas WHERE user_id=".$userId);
        $stmt->execute();
        $cnfFilas = $stmt->fetch(PDO::FETCH_ASSOC);
        if (!empty($cnfFilas['filas'])) {
            $sqlVer = "SELECT id_fila_chat, protocolo FROM tbl_chat_fila WHERE status_fila=1 AND contrato_id=".$contratoId." AND fila_id IN (".$cnfFilas['filas'].") AND (bko_resp IS NULL OR bko_resp='' OR bko_resp=0) ORDER BY id_fila_chat ASC LIMIT 1";
            $stmt = $PDO->prepare($sqlVer);
            $stmt->execute();
            $emFila = $stmt->fetch(PDO::FETCH_ASSOC) ?: [];
        }
