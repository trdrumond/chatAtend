...
$sql="SELECT resp_id, contrato_id, date_disp from tbl_tma_atend where date_out is null and resp_id=".$infoUser['id_user']." and (date_format(date_disp, '%Y-%m-%d')=curdate() or date_disp is null) order by id desc limit 1";
//echo "<br>".$sql;
...
if($infoAtend['resp_id']==''){
    ...
    $sql="UPDATE tbl_tma_atend SET date_disp=now() where resp_id=".$infoUser['id_user']." and date_out is null and fila_chat_id is null order by id desc limit 1";
    $stmt = $PDO->prepare($sql);
    $stmt->execute();
    $infoAtend['date_disp'] = date('Y-m-d H:i:s');
}

//echo "<br>Tabela tbl_tma_atend<br>";
//depurador($infoAtend);
...
//echo "<br>Tabela tbl_log_atendimento<br>";
//depurador($infoLog);
