...
$stmt = $PDO->prepare("UPDATE tbl_chat_fila SET ate_resp=? where id_fila_chat=?");
$result = $stmt->execute([$solId, $filaId]);

$stmt = $PDO->prepare("UPDATE tbl_chat_fila_secondary SET ate_resp=? where id_fila_chat=?");
$result = $stmt->execute([$solId, $filaId]);

if ($result == 1) {
    $stmt = $PDO->prepare("UPDATE tbl_chat_info SET dest_chat=? where fila_chat_id=?");
    $result = $stmt->execute([$solId, $filaId]);

    $stmt = $PDO->prepare("UPDATE tbl_chat_info_secondary SET dest_chat=? where fila_chat_id=?");
    $result = $stmt->execute([$solId, $filaId]);

    if ($result == 1) {
        $stmt = $PDO->prepare("UPDATE tbl_pend_info SET ate_resp=? where chat_id=?");
        $result = $stmt->execute([$solId, $filaId]);
