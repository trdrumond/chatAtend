...
final class MonitoraController
{
  ...
  /** @param list<string> $segmentos */
    ...
    private function atendimentosLote(): void
    {
        $dataInicio = trim((string) ($_GET['data_inicio'] ?? ''));
        $dataFim = trim((string) ($_GET['data_fim'] ?? ''));
        $contrato = trim((string) ($_GET['contrato'] ?? ''));
        ...
        $diasMaximo = (int) ($this->config['lote_periodo_maximo_dias'] ?? 31);
        $diffDias = (int) ((strtotime($dataFim) - strtotime($dataInicio)) / 86400) + 1;
        if ($diffDias > $diasMaximo) {
            MonitoraResponse::erro(
                400,
                'PARAMETROS_INVALIDOS',
                'Período máximo para lote é de ' . $diasMaximo . ' dias. Reduza o intervalo ou use paginação em /atendimentos.'
            );
        }

        $pagina = max(1, (int) ($_GET['pagina'] ?? 1));
        $porPagina = (int) ($_GET['por_pagina'] ?? ($this->config['lote_por_pagina_padrao'] ?? $this->config['por_pagina_padrao'] ?? 100));
        $maximo = (int) ($this->config['lote_por_pagina_maximo'] ?? $this->config['por_pagina_maximo'] ?? 500);
        $porPagina = max(1, min($porPagina, $maximo));

        MonitoraResponse::json($this->repo->listarAtendimentosCompletos(
            $dataInicio,
            $dataFim,
            $contrato,
            $fila,
