...
function infoAtend($PDO, $id_chat){
    ...
    $sql="SELECT date_format(data_hora, '%d/%m/%Y %H:%i:%s') as data_hora, rem_id, (SELECT concat(nome, ' ', sobrenome) from tbl_user where id_user=rem_id) as nome_envio, msg FROM tbl_chat_msg_secondary where chat_id=".$info['id_chat']." order by id_msg";
