...
function infoAtend($PDO, $id_chat){
    ...
    $chatId = (int) ($info['id_chat'] ?? 0);
    $sql="SELECT date_format(data_hora, '%d/%m/%Y %H:%i:%s') as data_hora, rem_id, (SELECT concat(nome, ' ', sobrenome) from tbl_user where id_user=rem_id) as nome_envio, msg FROM tbl_chat_msg_secondary where chat_id=? order by id_msg";
    ...
    return atendTxt($PDO, $info['id_chat'], $string);
}
    //echo "<br>----------<br>";
